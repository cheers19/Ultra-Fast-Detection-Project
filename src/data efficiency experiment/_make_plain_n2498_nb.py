"""Create plain Multires n=2498 protocol notebook."""
...
OUT = Path(__file__).resolve().parent / "plain_multires_n2498_NB.ipynb"
...
cells.append(
    md(
        r"""# Data-efficiency — Plain Multires, \(n_{\mathrm{train}}=2498\)
...
cells.append(md(r"## LR screening (plain, $\lambda=0$) + sanity curves"))

cells.append(
    code(
        r"""def screen_one_lr(lr: float) -> dict:
...
cells.append(md(r"## Final plain training with LR*"))

cells.append(
    code(
        r"""TAG_FINAL = lr_tag(LR_STAR) + "_final"
train_plain(
    TAG_FINAL,
    lr=LR_STAR,
...
cells.append(
    code(
        r"""plot_pulse_curves(hist, f"Plain n={N_TRAIN} λ=0 LR*={LR_STAR:g}")
...
cells.append(
    code(
        r"""# Shared test loader (seed+2), same for all campaign models
