"""Create Protocol v2 plain Multires notebook for n_train=866."""
...
OUT = Path(__file__).resolve().parent / "plain_multires_n866_v2_NB.ipynb"
...
cells.append(
    md(
        r"""# Data-efficiency v2 — Plain Multires, \(n_{\mathrm{train}}=866\)
...
Artifacts: `checkpoints/v2/n866_plain/` under this folder.
...
OUT = EXP / "checkpoints" / "v2" / "n866_plain"
...
cells.append(md("## Train plain (λ=0, LR=1e−3, PROTOCOL v2)"))
...
cells.append(
    code(
        r"""plot_pulse_curves(hist, f"Plain v2 n={N_TRAIN} λ=0 LR={LR:g}")
