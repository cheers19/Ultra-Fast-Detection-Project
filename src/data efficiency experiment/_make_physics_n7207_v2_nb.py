"""Create Protocol v2 Band-B physics Multires notebook for n_train=7207."""
...
OUT = Path(__file__).resolve().parent / "physics_multires_n7207_v2_NB.ipynb"
...
cells.append(
    md(
        r"""# Data-efficiency v2 — Physics Multires, \(n_{\mathrm{train}}=7207\) (Band B)

Follows **`PROTOCOL_v2.txt` / `PROTOCOL_v2.md`** §5.B (\(6000\le n<45000\)).
...
| Screen budgets | `max_steps=187500`, **`patience_steps=4700`** |
| \(\lambda\) Stage 1 | \(\{0.6,\ 1.8,\ 4.3\}\) |
| \(\lambda\) Stage 2 | only if 1.8 wins: \(\sqrt{1.8\cdot\lambda_{\mathrm{side}}}\) with \(\lambda_{\mathrm{side}}=\) **worse** edge |
| After \(\lambda^\*\) | exact resume from stop (**+2000** steps); official = best(screen+extension) |

Artifacts: `checkpoints/v2/n7207_physics/` (includes `*_train_state.pt` per screen).
...
OUT = EXP / "checkpoints" / "v2" / "n7207_physics"
OUT.mkdir(parents=True, exist_ok=True)
PLAIN_OUT = EXP / "checkpoints" / "v2" / "n7207_plain"
...
MAX_STEPS = 187500
# PROTOCOL v2 §5.B screen patience (overrides plain §2.1 patience for physics screens)
PATIENCE_STEPS = 4700
MAX_EPOCHS = max(1, math.ceil(MAX_STEPS / STEPS_PER_EPOCH))
EXTENSION_STEPS = 2000

# PROTOCOL v2 §5.B Stage 1
LAMBDA_COARSE = [0.6, 1.8, 4.3]
LAMBDA_EDGES = [0.6, 4.3]
LAMBDA_CENTER = 1.8
...
If Stage-1 winner is an edge (\(0.6\) or \(4.3\)): skip.
...
cells.append(
    md(
        r"""## Stage 4 — exact resume extension (+2000 steps)

Load `{TAG_STAR_SCREEN}_train_state.pt`, continue **exactly** from the screen stop
for `EXTENSION_STEPS=2000` more optimizer steps (no patience early-stop during
...
print(json.dumps(meta, indent=2))
plot_pulse_curves(hist, f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g} +ext")
plot_trace_curves(hist, f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g} +ext")
plot_grad_norms(hist, f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g} +ext", lam=LAM_STAR)
summarize_timings(hist, f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g} +ext")
print(
...
plt.suptitle(
    f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g} snapshots @ SNR={SNAPSHOT_SNR_DB:g} dB"
)
plt.tight_layout()
plt.show()
...
if FORCE_TEST_SWEEP or not sweep_path.exists():
    ...
    diag.run_and_save_test_snr_sweep(
        ...
        device=DEVICE,
        experiment_name=f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g}",
    )
else:
...
axes[0].set_title(f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g}: test L1 vs SNR")
axes[0].grid(True, alpha=0.3)
axes[1].errorbar(
...
axes[1].set_title(f"Physics v2 Band-B n={N_TRAIN} λ*={LAM_STAR:g}: test SIM vs SNR")
axes[1].grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
axes[1, 1].legend()
plt.suptitle(f"Physics v2 Band-B n={N_TRAIN}  λ*={LAM_STAR:g}  test[0] @ 10 dB")
plt.tight_layout()
plt.show()
