"""Create Protocol v2 plain Multires notebook for n_train=300."""
...
OUT = Path(__file__).resolve().parent / "plain_multires_n300_v2_NB.ipynb"
...
cells.append(
    md(
        r"""# Data-efficiency v2 â€” Plain Multires, \(n_{\mathrm{train}}=300\)
...
| Train SNR | U[0, 30] dB |
| Val SNR | discrete **\{âˆ’10, 0, 30\}** dB |
| Early stop | `patience_steps=800`, `max_steps=6400` (\(n\le 3000\) band; operator-adjustable) |
| Snapshots | every 2 epochs; fixed val sample @ SNR=10 dB |

Artifacts: `checkpoints/v2/n300_plain/` under this folder.

Run order: setup â†’ Helpers â†’ train â†’ curves â†’ snapshots â†’ SNR sweep â†’ test example â†’ summary.
"""
    )
)
...
OUT = EXP / "checkpoints" / "v2" / "n300_plain"
...
def summarize_timings(hist, name):
    ...
    print(f"=== {name}: meanÂ±std over epochs (ms/batch) ===")
    for k in keys:
        if k not in hist:
            continue
        x = np.asarray(hist[k], dtype=float) * 1e3
        print(f"  {k.replace('timing_', ''):22s}  {x.mean():7.3f} Â± {x.std():6.3f}")
...
def plot_pulse_curves(hist, title_prefix):
    ...
    axes[1].set_title(f"{title_prefix}: val pulse L1 (SNRâˆˆ{{-10,0,30}})")
...
def plot_trace_curves(hist, title_prefix):
    ...
    axes[1].set_title(f"{title_prefix}: val TRACE L1 (SNRâˆˆ{{-10,0,30}})")
    axes[1].set_xlabel("epoch")
    axes[1].grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
...
cells.append(md("## Train plain (Î»=0, LR=1eâˆ’3, PROTOCOL v2)"))
...
cells.append(md("## Training curves (pulse / TRACE / gradients / timings)"))

cells.append(
    code(
        r"""plot_pulse_curves(hist, f"Plain v2 n={N_TRAIN} Î»=0 LR={LR:g}")
plot_trace_curves(hist, f"Plain v2 n={N_TRAIN} Î»=0 LR={LR:g}")
plot_grad_norms(hist, f"Plain v2 n={N_TRAIN} Î»=0 LR={LR:g}", lam=0.0)
...
cells.append(md("## Snapshot evolution (val sample @ SNR=10 dB, every 2 epochs)"))

cells.append(
    code(
        r"""snap_path = OUT / f"{TAG}_snapshots.npz"
...
for row, si in enumerate(show_idx):
    ...
    axes[row, 0].set_xlabel("Delay Ï„ [fs]")
...
cells.append(
    code(
        r"""model = diag.load_trained_multires(OUT / f"{TAG}_model.pt", device=DEVICE)
model.eval()
...
axes[0, 0].set_xlabel("Delay Ï„ [fs]")
...
axes[0, 1].set_xlabel("Delay Ï„ [fs]")
