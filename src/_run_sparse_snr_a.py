"""Train Model A sparse-SNR {0,15,30}: λ=3 and λ=0; then test SNR sweeps."""
...
# Tee stdout/stderr to a log so background runs survive pipe truncation.
_LOG = Path(__file__).resolve().parent / "_sparse_snr_a_log.txt"
class _Tee:
...
TRAIN_SNR = [0.0, 15.0, 30.0]
SNR_SWEEP = np.arange(-10.0, 31.0, 5.0)

print("device", DEVICE)

for tag, lam in [
    ("model_a_raw_sparse_snr_lam3", 3.0),
    ("model_a_raw_sparse_snr_lam0", 0.0),
]:
...
for tag in ("model_a_raw_sparse_snr_lam3", "model_a_raw_sparse_snr_lam0"):
