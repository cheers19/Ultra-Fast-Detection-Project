"""Multires 2K noisy-train λ search (SNR 5–30 dB train, val @ 15 dB); scaled trace L1 loss."""
...
@dataclass
class NoisyTraceLossTrainResult:
...
def calibrate_trace_scale(
    ...
    *,
    ...
    """Median trace_l1_sum / pulse_l1_sum on clean batches (random init)."""
    model.eval()
    ratios: list[float] = []
    with torch.no_grad():
...
def train_multires_noisy_trace_loss_early_stop(
    ...
    *,
    ...
    val_snr_db: float,
    verbose: bool = True,
) -> NoisyTraceLossTrainResult:
    """Noisy input; loss = L1_pulse + λ·(L1_trace_sum / trace_scale) vs clean trace."""
    ...
    for epoch in range(max_epochs):
        ...
        if verbose:
            print(f"  reached max_epochs={max_epochs}; best epoch {best_epoch}", flush=True)

    if best_state is not None:
        model.load_state_dict(best_state)

    return NoisyTraceLossTrainResult(
        history=history,
...
def main() -> None:
    ...
    parser.add_argument("--train-snr-min", type=float, default=5.0)
    ...
    parser.add_argument(
        "--output",
        default="checkpoints/benchmark/multires_2k_noisy_trace_lambda.npz",
    )
    parser.add_argument(
        "--meta-json",
        default="checkpoints/benchmark/multires_2k_noisy_trace_lambda_meta.json",
    )
    parser.add_argument(
        "--sweep-output",
        default="checkpoints/benchmark/multires_2k_noisy_trace_lambda_cnn_sweep.npz",
    )
    parser.add_argument(
        "--ckpt-dir",
        default="checkpoints/benchmark/multires_2k_noisy_trace_lambda",
    )
    parser.add_argument(
        "--opt-checkpoint",
        default="checkpoints/benchmark/multires_2k_noisy_trace_lambda_opt.pt",
    )
    ...
    # Reconcile summary from per-lam checkpoints (robust to partial resume / concurrent runs).
    run_log = []
    best_val_l1 = np.full(n_lam, np.nan)
    best_epochs = np.full(n_lam, -1, dtype=np.int32)
    stopped_epochs = np.full(n_lam, -1, dtype=np.int32)
    ...
    if not args.skip_sweep and opt_src.exists():
        print(f"\nSNR sweep on test (n={int(args.n_test)}) …", flush=True)
        model = build_model(n, device, model_name="multires")
        ...
        sweep = run_cnn_snr_sweep(
            ...
            test_loader,
            SNR_SWEEP_DB,
            experiment_name="multires_2k_noisy_trace_lambda",
            verbose=True,
        )
