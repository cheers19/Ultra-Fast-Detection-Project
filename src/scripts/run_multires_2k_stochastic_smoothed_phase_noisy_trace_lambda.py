"""Multires 2K on smoothed-phase-noise SASE pulses — noisy-train λ search (SNR 0–30 dB train, val @ 15 dB)."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="checkpoints/benchmark/stochastic_smoothed_phase_multires_2k_noisy_trace_lambda.npz",
    )
    parser.add_argument(
        "--meta-json",
        default="checkpoints/benchmark/stochastic_smoothed_phase_multires_2k_noisy_trace_lambda_meta.json",
    )
    parser.add_argument(
        "--sweep-output",
        default="checkpoints/benchmark/stochastic_smoothed_phase_multires_2k_noisy_trace_lambda_opt_sweep.npz",
    )
    parser.add_argument(
        "--baseline-sweep-output",
        default="checkpoints/benchmark/stochastic_smoothed_phase_multires_2k_noisy_trace_lambda_baseline_sweep.npz",
    )
    parser.add_argument(
        "--ckpt-dir",
        default="checkpoints/benchmark/stochastic_smoothed_phase_multires_2k_noisy_trace_lambda",
    )
    parser.add_argument(
        "--opt-checkpoint",
        default="checkpoints/benchmark/stochastic_smoothed_phase_multires_2k_noisy_trace_lambda_opt.pt",
    )
    ...
    stoch_grid = SmoothedPhaseStochasticPulseConfig(n=n)
    ...
    print(
        f"smoothed-phase stochastic pulses: {int(args.n_train)} train / {int(args.n_val)} val / {int(args.n_test)} test",
        flush=True,
    )
    print(
    ...
    for li, lam in enumerate(lambda_grid):
        ...
        log_entry = {
            ...
            "pulse_kind": "stochastic_smoothed_phase",
        }
    ...
    if not args.skip_sweep:
        ...
        if opt_src.exists():
            print(f"\nSNR sweep (λ*) on smoothed-phase test (n={int(args.n_test)}) …", flush=True)
            model = build_model(n, device, model_name="multires")
            opt_data = torch.load(opt_src, map_location=device, weights_only=False)
            ...
            sweep = run_cnn_snr_sweep(
                ...
                test_loader,
                SNR_SWEEP_DB,
                experiment_name="stochastic_smoothed_phase_multires_2k_trace_lambda_opt",
                verbose=True,
            )
        ...
        if baseline_src.exists():
            ...
            sweep_base = run_cnn_snr_sweep(
                ...
                test_loader,
                SNR_SWEEP_DB,
                experiment_name="stochastic_smoothed_phase_multires_2k_baseline",
                verbose=True,
            )
